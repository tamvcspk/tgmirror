...
engine/   endpoints (source/destination rules), jobs (create/resolve/vet), planner, batcher, preview, strategies (copy / reupload), flood (`FloodGuard`: pacing + FloodWait handling for reads and writes), reconcile, runner
filters/  model, parser (YAML + flags), pushdown (`plan_read`), matcher (pure, client side)
store/    schema.sql, db (`Store`: the only place with SQL), jobs, msgmap, floodlog, limiterstate
