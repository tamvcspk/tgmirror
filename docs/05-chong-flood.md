...
| `max_requests` | 8 | Số request truyền file đang bay cùng lúc trong cả tiến trình, chia chung cho tải xuống và tải lên (`RequestBudget`, `core/pool.py`); 0 tắt pool (Telethon tự tải, một request một lúc). Ngân sách **bắt đầu ở 2**, tăng một bậc sau mỗi 32 part liền không gặp lực cản (và không trong 30 giây sau lần gặp), **giảm một nửa** khi Telegram phản kháng. Mặc định đổi 4 → 8 (2026-09-23): `upload_connections = 8` / `max_requests = 8` (một request mỗi kết nối), một file tại một thời điểm, không có tải xuống nào chạy cùng, đo sạch **hai lần liền** (~24–28 MB/s); vượt quá theo bất kỳ hướng nào (nhiều request hơn kết nối, hay nhiều kết nối hơn 8) đều vỡ cả hai lần thử — không còn là phỏng đoán, xem "Kế hoạch tăng tốc" trong `06-lo-trinh.md` |
| `upload_connections` | 8 | Số kết nối (tối đa 16, đo an toàn tới 8) **riêng** mà phần tải lên chia part ra, tới cùng DC (dùng chung auth key), không tính kết nối chính. Không tạo được thì dùng ít hơn, không tạo được cái nào thì dùng kết nối chính. Tải xuống luôn một kết nối **riêng** của nó (spike 12: thêm kết nối không nhanh hơn). Dữ liệu lớn không bao giờ đi chung kết nối chính với các lời gọi đọc và đăng (lần chạy thật đầu tiên bị 429 khi tải xuống và tải lên cùng đi trên kết nối chính). Mặc định đổi 2 → 8 cùng lý do với `max_requests` ở trên; 16 kết nối đã vỡ (TIMEOUT) trong cùng lần đo, nên đừng vượt 8 |
