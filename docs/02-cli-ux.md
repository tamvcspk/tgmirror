...
2. Chọn đích            ← (a) có sẵn, bất kỳ loại nào (phase 8: không còn phải cùng loại nguồn), có quyền post
                           (b) Tạo mới: nhập tên [+ about] [+ copy avatar]; luôn cùng loại nguồn (group → supergroup vì
...
3. Chọn filter          ← "Không lọc" / "Chọn tiêu chí" (checkbox media types, rồi hỏi hashtag, từ khóa,
                           ...
                           sai thì hỏi lại (tối đa 3 lần). Chỉ hỏi khi cần wizard cho cả phần còn lại (thiếu --src
...
4. Cách sao chép       ← luôn hỏi **mode** (`--mode`): tự động (mặc định) / chỉ forward (`copy`) / tải xuống rồi tải lên lại
                           ...
                           Nguồn forum → đích không phải forum (phase 8, "topic_loss"): nếu mode viết lại được nội dung,
                           hỏi thêm "Giữ tên topic dưới dạng hashtag?" (mặc định có); nếu không (copy, hoặc auto không đổi
                           caption), hỏi xác nhận riêng "Toàn bộ topic sẽ bị bỏ, tiếp tục?" (mặc định không; `--yes` đủ
                           để đồng ý, không có terminal thì mã 2) — xem `01-kien-truc.md`, "Đích khác loại nguồn"
