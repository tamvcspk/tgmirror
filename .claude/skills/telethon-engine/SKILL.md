...
description: How tgmirror talks to Telegram through Telethon — client setup, login, error mapping, listing joined channels, creating a destination channel, iterating source messages, server-side copy (forward with drop_author), album handling, the reupload fallback, and sending a unit read from a backup directory (no live Telethon message) for `tgmirror restore`. Use when writing or changing anything in core/telethon_gateway.py, engine/copy.py, engine/reupload.py or engine/backup_reader.py, or when a Telethon API detail is in question.
---
...
## Backup (`export_unit`, phase 11a)

`TelethonGateway.export_unit` reuses `_read_unit`/`_download`/`_has_file` from strategy B but never deletes files, and returns `ExportedMessage`s (self-contained: HTML text via `telethon.extensions.html.unparse(message.message, message.entities)`, plus `ExportedMedia` — filename/mime/size/duration for a file, or `_poll_media`/`_geo_media`/`_contact_media`/`{GAME,INVOICE}` payload for self-contained media) instead of a `Prepared`, because `engine/backupdir.py` writes `messages.jsonl` without touching Telethon types at all (hard rule 8). Design and what is not yet verified on a real account: `docs/01-kien-truc.md`, "Backup ra đĩa"; state model: `docs/04-state-checkpoint.md`, "Backup".

## Restore (`_send_from_backup_*`, phase 11b)

`send_prepared`/`upload_prepared` branch on `isinstance(prepared.handle, FromBackup)` before the usual `assert isinstance(handle, _Fetched)`: a restore's unit has no live Telethon message to rebuild from, only `ExportedMessage`/`ExportedMedia` (what `messages.jsonl` kept) and files already on disk. `upload_prepared` is a no-op for this handle (no pre-upload optimization for v1). `send_prepared` (`_send_from_backup` → `_send_backup_one`/`_send_backup_album`) builds the call directly instead of copying attributes from a live document:

- File kinds (single message): `client.send_file(path, ...)` with `voice_note=`/`video_note=`/`force_document=` set from `ExportedMedia.kind` (not from a copied `attributes` list) — hachoir auto-detects duration as it already does for a live reupload's single files. A single file big enough (`_pooled_upload_path`, same `BIG_FILE`/`min_bytes` thresholds as strategy B, `MediaKind.PHOTO` excluded like the live path) still goes through `_upload_parallel`/`RequestBudget` instead of Telethon's own single-connection uploader — added 2026-09-26 after a real restore measured single-connection speed (~2.7 MB/s) because the first version always called `send_file(path)` directly.
- Album: shares `_upload_album_member`/`_post_multi_media` with a live reupload's own album (`_album_media`/`_send_album`) — **one core, one `TransferSettings`/`[limits]` config for both**, added 2026-09-26 alongside the single-file fix, at the user's explicit request not to let clone and restore drift onto separate upload implementations. `_album_media` (live) and `_backup_album_member` (restore) are now thin adapters that pull `path`/`mime`/`attributes`/`thumb` from a live `_Item` or an `ExportedMessage` respectively and call the same `_upload_album_member` (pools a big member exactly like a single file does); `_send_album`/`_send_backup_album` both post through `_post_multi_media`. Restoring an album no longer goes through Telethon's own multi-file `send_file([...])`, so it also gets `_album_progress` reporting it did not have before. Sticker/gif fidelity from a bare file is unverified (`docs/06-lo-trinh.md`, "Phase 11b — ghi chú").
- Poll/quiz: a fresh `types.Poll`/`types.InputMediaPoll` built from `poll_question`/`poll_options` (never reused from a live object — there is nothing to reuse); `correct_answers` only when `poll_correct_option` was captured (a quiz backed up unanswered stays unsupported, exactly like `plan_unit`'s live-quiz rule).
- Geo/venue/contact: fresh `InputMediaGeoPoint`/`InputMediaVenue`/`InputMediaContact` (`_backup_self_contained_media`) — `utils.get_input_media` passes an already-`InputMedia*` object straight through, so these need no live media to convert from either.
- Text: `telethon.extensions.html.parse` recovers text+entities from `ExportedMessage.text_html` — the exact inverse of `export_unit`'s `html.unparse`. Caption rewriting (`rewrite_caption`) uses `_source_names_from_id(handle.src_id)` instead of `_source_names(message)`: no username to check against (a backup never records one), only the numeric `t.me/c/<id>/...` form.
- Album: one `client.send_file(peer, [paths...], caption=[...], formatting_entities=[[...], ...])` call, letting Telethon's own multi-file path build `InputSingleMedia`/`SendMultiMediaRequest` (no manual `UploadMediaRequest` per member as strategy B's live album path does) — always ≤ `ALBUM_MARGIN` (10) files, under Telegram's own grouped-media limit, so the >10 chunking Telethon has for a bigger list never triggers. No progress callback exposed for this path (v1 cut, like Phase 11a's own no-op transfer line).

`FakeGateway` needs no matching branch: its `send_prepared`/`upload_prepared` only ever look at `prepared.files`, never `.handle`.
