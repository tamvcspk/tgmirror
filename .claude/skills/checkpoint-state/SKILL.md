...
One SQLite file (WAL). `jobs` holds config + status + `cursor_src_id` (highest source id fully finished) + control flag. `msg_map` maps `src_msg_id → dst_msg_id` per job with status `pending|done|failed|skipped` (`skipped` = unsupported type such as game/invoice, with `reason='unsupported:<kind>'`; `retry` ignores it). Filter-skipped messages are **not** stored — they only advance the cursor and the `skipped_filter` counter (credited per batch: `Batch.skipped`/`Batch.upto`, committed with the batch or via `advance_cursor(extra_stats=...)`; the batcher flushes a batch after 500 skips so long stretches still save progress and hear pause/stop). Forum jobs also keep `topic_map` (`src_topic_id → dst_topic_id`) and `msg_map.src_topic_id`; creating a destination topic and saving its `topic_map` row happen in one transaction. `flood_log` and `limiter_state` support tuning and persistence (`limiter_state` holds the limiter's `delay`, local `day` and `sent_today` per `jobs.account`; `commit_batch(limiter=...)` writes it in the batch's transaction, `save_limiter_state` after a flood). `jobs.options_json` is `JobOptions`: `batch_size`, `dst_base_id` (the destination's newest message id at creation, so reconcile never scans an old destination) and `pushdown` (`false` = read everything, filter locally); unknown keys are ignored. `jobs.filters_json` is the canonical `FilterSpec.to_json()` (`{}` = no filter).
...
3. `cursor_src_id` is monotonic and only advances past a batch with no `pending` rows left. Never move it backwards, except `Store.replace_filters` (`run --refilter`): one transaction sets the new `filters_json`, cursor 0 and clears `skipped_filter`, and relies on `msg_map` to skip `done` items; it refuses a job another process is running.
...
3. Iterate `iter_messages(src, min_id=cursor_src_id)` (ascending) with the stored filters (`plan_read`: pushdown, or a full scan with `options.pushdown = false`).
...
- `FloodWait` within `max_auto_wait`: wait and repeat the same call, `pending` stays. `FloodWait` too long or 5 in a row, `PeerFlood`, `NoPermission`, `ForwardsRestricted`, other rejections, pause/stop during a flood wait: nothing was created ⇒ delete the batch's `pending`, then stop the job. The daily cap fires before the write-ahead (nothing pending): job `waiting_flood`, `fail_reason='daily_cap'`.
...
- `cursor_src_id` never decreases (except `--refilter`); filter-only-skipped stretches still advance it and a pause gets through them; `skipped_filter` is counted once across a pause/resume and through the `PerMessage` split (`tests/integration/test_runner_filters.py`).
