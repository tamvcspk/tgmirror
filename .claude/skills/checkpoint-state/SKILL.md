...
- Strategy B: one unit per batch, so a pending batch is one unit and reconcile compares it as usual. `prepare` runs **before** the write-ahead, so a refused fetch (`PerMessage`) leaves nothing pending: the unit is written `failed` at once. A unit left out on purpose (`DROP`) is recorded with `begin_batch` + `commit_batch` and no Telegram call between. A `FloodWait` while sending repeats the same call with the same downloaded files.
