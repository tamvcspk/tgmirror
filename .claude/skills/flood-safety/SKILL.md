...
- [ ] `seconds > max_auto_wait`, or 5 floods in a row on one call → refused batch's `pending` deleted, job `waiting_flood`, `resume_at` set, exit 3 (keep waiting only with `--wait`, never past the 5-in-a-row stop).
