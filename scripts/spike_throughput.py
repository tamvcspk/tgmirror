...
needed? Does Telegram answer FloodWait when many parts are asked at once?
    ...
    .venv\Scripts\python.exe scripts\spike_throughput.py both @src 42 --up-connections 2
...
async def both(args: argparse.Namespace) -> None:
    """A download and an upload at the same time, as the real run does them."""
    async with session() as (client, _):
        peer = await chat_id(client, args.src)
        message = (await client.get_messages(peer, ids=[args.message]))[0]
        ...
        print(
            ...
            f"flight on {args.up_connections} conn, {mb(size)} each way"
        )
        for label, together in (("alone", False), ("together", True)):
            ...
            for _ in range(args.repeat):
                ...
                up = pool(
                    ...
                    args.up_inflight,
                    args.up_connections,
                    lambda i, file_id=file_id: SaveBigFilePartRequest(file_id, i, up_parts, blob),
                )
                if together:
...
def main() -> None:
    ...
    both_parser.add_argument("--up-connections", type=int, default=2)
